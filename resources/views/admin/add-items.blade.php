...
<div class="uk-container uk-container-small">
		<ul uk-tab>
	    <li><a href="#">{{__("Add Item")}}</a></li>
	    <li><a href="#">{{__("Import Data")}}</a></li>
		</ul>
	    ...
	    <li>
				...
				{!!Form::open(['url'=>'/admin/import-item','files'=>true,'id'=>'import-form'])!!}
				{!!Form::label('Select Shop')!!}
				<select name="shop" class="uk-select uk-border-rounded uk-margin-small">
					<option value="">--{{__("Select Shop")}}--</option>
					@foreach($bouti as $values)
					<option value="{{$values->localisation}}">{{$values->localisation}}</option>
					@endforeach
				...
				<div class=" uk-margin-small">
					{!!Form::label(__("Choose File"))!!}
					<div uk-form-custom>
						{!!Form::file('fichier')!!}
		          <button class="uk-button-primary uk-padding-small uk-border-circle uk-icon-link" uk-icon="icon:pull" type="button" tabindex="-1"></button>
		      </div>
				</div>
				{!!Form::submit(__("Submit"),['class'=>'uk-button uk-button-primary uk-width-1-1@s uk-width-1-6@m uk-border-rounded uk-box-shadow-small'])!!}
				{!!Form::close()!!}
			</li>
		</ul>
