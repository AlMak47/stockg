	...
	<h3 class="uk-h3"> {{__("List Shop")}} </h3>
	    ...
	    <tbody>
	    	...
	    	<tr>
	    		...
	    		<td><a class="bloque-user uk-button-small uk-button uk-text-capitalize uk-button-danger uk-border-rounded" href="#block-user" id="{{$values->username}}" uk-toggle="target: #my-id"><span uk-icon="icon:lock"></span> bloquer</a></td>
					@else
	    		<td><a class="bloque-user uk-button-small uk-button-default uk-button uk-text-capitalize uk-alert-success uk-border-rounded" href="#unblock-user" id="{{$values->username}}" uk-toggle="target: #my-id"><span uk-icon="icon:unlock"></span> debloquer</a></td>
					@endif
	    		<td><a href=""></a></td>
	    	</tr>
	    		@endforeach
